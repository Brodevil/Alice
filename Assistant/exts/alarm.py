...
def speak(*args) -> None:
    ...
    engine.setProperty(
        "voice", engine.getProperty("voices")[int(environ.get("VoiceNumber", 1)) - 1].id
    )
    print(f"{Client.ASSISTANT_NAME} : {' '.join(args)}", "\n")
...
def notifier(reason: str, string: str, ico: str) -> None:
    ...
    """
    try:
        notification.notify(title=string, message=reason, app_icon=ico, timeout=12)
    except Exception:
        pass
    ...
    while True:
        ...
        if presentTime == remindTime:  # noqa
            notifier(
                ...
                ico=path.join(
                    Client.ALICE_PATH, "Assistant\\resources\\Images\\time-out.ico"
                ),
            )
            winsound.Beep(frequency=2500, duration=4000)
            speak(
                f"Time Up, You had told me to remind you after {magnitude}",
                " hours " if is_hour else " minutes",
