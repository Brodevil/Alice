    ...
    if location is None:
        ...
        if local_info != None:
            ...
            try:
                ...
                response = json.loads(response.text)
                return f"It seemed to be approximately {int(response['main']['temp'] - 273.15)} degree Celcius! I guess its like {response['weather'][0]['main']} climate, and the wind speed is feels like {response['wind']['speed']*3.6} kilometer per hour"
            except ConnectionError:
                return None
            except IndexError:
