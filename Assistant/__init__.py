...
def logic(queary: str) -> None:
    ...
    if "search" in queary and "on wikipedia" in queary:
        ...
        if "type" in queary:
            alice.speak(
                f"{Client.GENDER}! You can ofcourse type the message on your own in the terminal!"
            )
    ...
    # --------------------------------------------work with GUI or windows :-----------------------------------------
        ...
        "screen" in queary
        and "bright" in queary
        or "display" in queary
        and "bright" in queary
    ):
        try:
            if queary.isalnum():
    ...
    # ----------------------- make Alice to type, Keyboard features, working with keyboards------------------------------
        ...
        alice.speak(
            f"{Client.GENDER}! You start to speak I will type that And then to quit plz say quite or close."
        )
        string = str()
        while (
            "stop" not in string.lower()
            and "quit" not in string.lower()
            and "close" not in string.lower()
        ):
            if string != "None":
        ...
        "play" in queary and "keyboard" in queary and "record"
    ):  # play my recorded keyboard/play keyboard recording/etc
        try:
            globals()["keyRecorded"] = globals()["keyRecorded"]  # noqa
        except NameError:
            alice.speak(
                f"{Client.GENDER}! there is no keyboard Activity available till now"
            )
        else:
            alice.speak(
    ...
    # -------------------------------------Music---------------------------------------------------
        ...
        "play" in queary and "music" in queary or "song" in queary and "music" in queary
    ):
        ...
        "brown munde" in queary
    ):  # this function is just for my self i.e. For Abhinav personal songs
        try:
            os.startfile(
        ...
        if Client.FAVOURITE_MUSIC is not None:
            ...
            alice.speak(
                f"{Client.GENDER}! Please run the documentation of Alice and then "
            )
    ...
    # --------------------------------------- Natural Talks/ Fun commands : ----------------------------
        ...
        "hello" in queary or queary == "alice" or "your" in queary and "name" in queary
    ):
        alice.speak(
        ...
        "good afternoon" in queary
        or "good evening" in queary
        or "good morning" in queary
    ):
        wish = alice.goodWish
        ...
        alice.speak(
            f"The Current Time Of {place} is {networks.quick_google_search(f'Current time of {place}')}"
        )

    elif "time" in queary:
        alice.speak(
            f"Its {datetime.datetime.now().strftime('%I:%M %p')} {Client.GENDER}!"
        )
        ...
        "what's my name" in queary
        or "what is my name" in queary
        or "who i am" in queary
    ):
        alice.speak(
            f"You are {alice.name} {Client.GENDER}!, But why did you are asking this?"
        )
        ...
        os.startfile(
            os.path.join(Client.ALICE_PATH, "Assistant\\resources\\images\\Ribbons.scr")
        )
    ...
    # ------------------------------------- Work with System, Info, features :------------------------------------------
        ...
        if battery is not None:
            ...
            alice.speak(
                f"Something went wrong {random.choice(ERROR_REPLIES)}. I think you are in desktop"
            )
        ...
        "internet speed" in queary
        or "network speed" in queary
        or "download" in queary
        and "speed" in queary
        or "upload" in queary
        and "speed" in queary
    ):
        ...
        alice.speak(
            f"{Client.GENDER}! You Current IPv4 Address is : {networks.ip_address()}"
        )
    ...
    # ---------------------------------- make Alice to speak, Test to Speech features --------------------------------
        ...
        "start following m" in queary or "repeat my self" in queary
    ):  # start following me or my voice both starts from m
        alice.speak(
            "I will be now following your while repeating yourself, say stop to quit this"
        )
        sentence = str()
        while (
            "stop" not in sentence
            and "quit" not in sentence
            and "break" not in sentence
        ):
            if sentence != "None":
        ...
        "spell" in queary and "text" in queary or "speak" in queary and "text" in queary
    ):
        ...
        "read book" in queary
        or "audiobook" in queary
        or "speak pdf" in queary
        or "read pdf" in queary
    ):
        ...
        if "increase" in queary or "more" in queary or "add" in queary:
            rate = alice.takeCommand(
                "Please tell How much should I increase the Voice Rate!"
            )

        elif "decrease" in queary or "less" in queary and "slow":
            rate = alice.takeCommand(
                "Please tell How much should I decrease my Voice Rate!"
            )
        ...
        if not rate.isalnum() or rate == "None":
            alice.speak(
                f"{Client.GENDER}! You should just speak number for Voice Rate for changes, Please try again"
            )
            return
    ...
    # ------------------------------------------ local info work with INTERNET and APIs -----------------------------
        ...
        "my location" in queary
        or "current location" in queary
        or "where i am" in queary
    ):
        alice.speak(
        ...
        if topTen is not None:
            for index, articles in enumerate(topTen):
                alice.speak(
                    "Moving On " + "a" if index == 0 else "another" + " fresh news!"
                )
                alice.speak(
            ...
            alice.speak(
                f"{random.choice(ERROR_REPLIES)}, Check your INTERNET_CONNECTION connection {Client.GENDER}!"
            )
        ...
        alice.speak(
            f"The Current Temperature " + f"of {Client.CITY}"
            if not len(place)
            else "" + f" is {temp}"
        )
        ...
        country = alice.takeCommand(
            "Please tell the country name which you want to get the corona cases"
        )
    ...
    # ------------------------------------------ reminders & Alarms ----------------------------------------------
        ...
        queary = queary.split(
            "remind me after " if "remind me after" in queary else "wake up me after"
        )[1]
        magnitude = int(queary.split()[0])
        unit = queary.split()[1]
        alice.speak(
            f"Okay {Client.GENDER}! I will be reminding you after {magnitude} {unit}!"
        )
        try:
            ...
            pourpose = (
                queary.split("so that ")[1].replace("i", "you").replace("my", "your")
            )
            ...
            globals()["side_reminder"].append(
                mp.Process(target=alarm.reminderAlarm, args=(magnitude, unit, pourpose))
            )
        globals()["side_reminder"][-1].start()

    elif "alarm" in queary:
        alice.speak(
            f"{Client.GENDER}! Please type the alarm time in 24 hour format, in the terminal!"
        )
        alarm_time = input("Enter the Alarm time in 24 hour Format : \t")

        if ":" not in alarm_time:
            alice.speak(
                f"Alarm Time format is wrong, Expected in 24 hour format, Please try again!"
            )
            return

        alarm_time = str(datetime.datetime.strptime(alarm_time, "%H:%M"))[11:16]
        alice.speak(
            f"Successfully set the Alarm of {alarm_time}, remind you soon {Client.GENDER}"
        )
        globals()["side_alarm"].append(
            mp.Process(target=alarm.alarm, args=(alarm_time,))
        )
        globals()["side_alarm"][-1].start()

    elif "stop" in queary and "remind" in queary:
        alice.speak(
            f"Okay {Client.GENDER}! I will not remind you for you daily task further"
        )
    ...
    # --------------------------------- work with login folder : i.e.work with E commerce websites accounts ------------
        ...
        try:
            ...
            if content == "None" or "let me" in content and "typ":
                alice.speak(
                    "Sorry, I didn't get that!, Can you please type the message in the terminal!"
                    if content == "None"
                    else f"Okay {Client.GENDER}! You can ofcourse type the message on your own in the terminal!"
                )
                content = input(
                    f"Enter the Message/Content of the Email {Client.GENDER}! : \t"
                )
        ...
        try:
            userName = alice.takeCommand(
                "To whom you want to send the Whatsapp Message!"
            )
            for _ in Contacts.contactNumber.keys():
                if userName.split()[0] in _.split():
                    ...
                    if contact_num is None:
                        alice.speak(
                            f"{Client.GENDER}! {_}'s Phone number had not been there in your contact Exel file!"
                        )
                        return
                    break
                ...
                contact_num = input(
                    "Enter the Contact Number including Country code  :\t"
                )
            ...
            pywhatkit.sendwhatmsg_instantly(
                contact_num.replace(" ", ""), content, wait_time=1
            )
            time.sleep(8)
            keyboard.press("enter")
        except Exception:
    ...
    # ----------------------------  Launching the software stuffs --------------------------------------------------
        ...
        app = workWithFiles.openApplication(
            applicationName, Client.APPLICATIONS_SHORTCUTS_PATH
        )
        if app is not None:
